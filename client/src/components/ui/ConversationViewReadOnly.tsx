        ...
        return (
          <div key={`msg-${index}`} className={`my-3 rounded-lg p-4 ${vendorColorClass} shadow-sm`}>
            ...
            {msg.referenceImages && Array.isArray(msg.referenceImages) && msg.referenceImages.length > 0 && (
              <div className="bg-white rounded-lg p-3 border mb-3">
                <div className="text-sm font-medium mb-2">참고 이미지:</div>
                <div className="flex flex-wrap gap-2">
                  {msg.referenceImages.map((img: string, imgIndex: number) => (
                    <img 
                      key={`ref-img-${imgIndex}`}
                      src={img} 
        ...
        >
          ...
          <div className="chat-message-container whitespace-pre-wrap break-words text-sm">
            ...
            {msg.role === 'vendor' && msg.referenceImages && Array.isArray(msg.referenceImages) && msg.referenceImages.length > 0 && (
              <div className="mt-3">
                <div className="text-xs font-medium mb-1 text-gray-600">참고 이미지:</div>
                <div className="flex flex-wrap gap-2">
                  {msg.referenceImages.map((img: string, imgIndex: number) => (
                    <img 
                      key={`general-ref-img-${imgIndex}`}
                      src={img} 
                      alt={`참조 이미지 ${imgIndex + 1}`}
                      className="max-w-[150px] max-h-[150px] object-contain rounded-md border border-gray-200"
                      onClick={() => window.open(img, '_blank')}
                      style={{ cursor: 'pointer' }}
                    />
                  ))}
